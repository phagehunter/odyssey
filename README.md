...
A single-page digital-humanities dashboard for the visual literary analysis of Homer's *Odyssey*, combining **distant reading** (computational visualization) with **close reading** (textual commentary, line citations, and the full Samuel Butler translation readable alongside every view).
...
A **global book slider** (1–24) and **group toggles** (Ithacan loyalists, suitors coalition, divine entities, external civilizations/monsters) re-render every view in real time. A **playback control** in the network view animates the epic book by book — watch the interaction network grow cumulatively or see each book's cast in isolation. Clicking any edge, path, waypoint, or node populates the **Commentary panel** with summaries, thematic tags (xenia, nostos, metis…), and Greek-text line citations — each citation linking into the **Text tab**, a full in-app reader of the Butler translation (1900, public domain) with per-book navigation.

All data is embedded as TypeScript constants (`src/data/`); the text is served as static per-book JSON generated by `node scripts/build-text.mjs` from `odyssey.mb.txt` — no backend, no API keys.
